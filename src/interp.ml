...
let rec interp_ast (ast: expr) = match ast with
  ...
  | BinOp (op, e1, e2) -> (
    ...
    (* | Mod -> if v2 = 0.0 then raise (RunTimeError "division by zero") else v1 %. v2 *)
    | Mod -> -1.0
  ...
  (*     | SubEq  -> v -. v' *)
  (*     | MulEq  -> v *. v' *)
  (*     | DivEq  -> if v' = 0.0 then raise (RunTimeError "division by zero") else v /. v' *)
  (*     | ModEq  -> if v' = 0.0 then raise (RunTimeError "division by zero") else v mod_float v' *)
...
let interp_prog (ast: expr list) = match ast with
  (* input will be an expr list *)
  | exprs -> List.fold_left (fun _ e -> interp_ast e) 0.0 exprs
